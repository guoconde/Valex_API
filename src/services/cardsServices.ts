...
export async function activate(
  ...
  if (password.length !== 4)
    throw error.UnprocessableEntity("Password must have 4 digits.");
...
export function verifyExpirationDate(card: repository.Card) {
  ...
  if (expirationDate.diff() > 0) throw error.Forbidden("Card is expired.");
}
...
export async function verifyPassword(cardId: number, password: string) {
  ...
  if (!passwordValidation) throw error.Unauthorized();
}

function verifyActive(card: repository.Card) {
  if (card.password)
    throw error.ConflictSpecificMessage("Card is already active");
...
async function verifyType(
  ...
  if (!transactionTypes.includes(cardType))
    throw error.Forbidden("Invalid card type.");

  const doesCardExist = await repository.findByTypeAndEmployeeId(
    cardType,
    employeeId
  );
  if (doesCardExist) throw error.Conflict(`Employee's ${cardType} card`);
}
